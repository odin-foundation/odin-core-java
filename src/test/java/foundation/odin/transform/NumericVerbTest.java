...
class NumericVerbTest {
    ...
    @Test
    void registry_HasAllNumericVerbs() {
        var reg = new VerbRegistry();
        var verbCtx = new TransformEngine.VerbContext();
        var verbs = new String[]{ "add", "subtract", "multiply", "divide", "abs", "round", "negate",
        ...
        for (var verb : verbs) {
            try { reg.invoke(verb, new DynValue[0], verbCtx); } catch (Exception ignored) { }
        }
    }

    @Test
    void registry_HasAllMathVerbs() {
        var reg = new VerbRegistry();
        var verbCtx = new TransformEngine.VerbContext();
        var verbs = new String[]{ "log", "ln", "log10", "exp", "pow", "sqrt" };
        for (var verb : verbs) {
            try { reg.invoke(verb, new DynValue[0], verbCtx); } catch (Exception ignored) { }
        }
    }
    ...
    @Test
    void registry_HasAllFinancialVerbs() {
        var reg = new VerbRegistry();
        var verbCtx = new TransformEngine.VerbContext();
        var verbs = new String[]{ "compound", "discount", "pmt", "fv", "pv", "rate", "nper", "npv", "irr", "depreciation" };
        for (var verb : verbs) {
            try { reg.invoke(verb, new DynValue[0], verbCtx); } catch (Exception ignored) { }
        }
    }
    ...
    @Test
    void registry_HasAllStatVerbs() {
        var reg = new VerbRegistry();
        var verbCtx = new TransformEngine.VerbContext();
        var verbs = new String[]{ "std", "variance", "stdSample", "varianceSample", "median", "mode",
    ...
    @Test
    void registry_HasAllDateTimeVerbs() {
        var reg = new VerbRegistry();
        var verbCtx = new TransformEngine.VerbContext();
        var verbs = new String[]{ "today", "now", "formatDate", "parseDate", "formatTime",
